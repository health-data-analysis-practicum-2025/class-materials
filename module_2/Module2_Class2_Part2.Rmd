...
Notice that now only `r sum(is.na(rename$drink))` respondents are missing a value for the new `drink` variable -- we have appropriately included those who don't drink (and therefore have a drinking frequency of 0 drinks per week) with those who do drink.
...
This cutoff would mean that `r round(mean(rename$drink < 1, na.rm = TRUE) * 100, 1)`% of respondents would be coded 0 and the rest would be coded 1. 
...
```{r boxhyage, fig.width=4, fig.height=4}
p1 <- hy_df %>% 
...
```{r multipanel, fig.width=11, fig.height=5}
p2 <- hy_df %>% 
...
```{r multipanel2, fig.width=11, fig.height=5}
p3b <- hy_df %>% 
...
Let's finish by looking at the relationship between hypertension and drinking using both of our derived drinking variables: `drink` and `drinkcat`. Do you see any evidence of an association between drinking and hypertension?

```{r multipanel3, fig.width=11, fig.height=5}
p5 <- hy_df %>% 
  drop_na(drink) %>% 
